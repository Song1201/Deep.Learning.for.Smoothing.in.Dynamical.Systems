...
initState = tf.placeholder(tf.float32,(None,1)) 
#%%   
nextState = initState
hRight = []
# where I stop 
for t in range(1,nTimeSteps+1):
    inputAndState = tf.concat((tf.reshape(xInput[:,-t],(-1,1)),nextState),axis=1)
    wHidden = tf.Variable(np.random.normal(0,0.1,(2,hiddenSize)),dtype=tf.float32)
    bHidden = tf.Variable(np.random.normal(0,0.1,(hiddenSize)),dtype=tf.float32)
    wOutput = tf.Variable(np.random.normal(0,0.1,(hiddenSize,1)),dtype=tf.float32)
    bOutput = tf.Variable(np.random.normal(0,0.1,(1)),dtype=tf.float32)
    nextState1 = tf.nn.leaky_relu(tf.matmul(inputAndState,wHidden)+bHidden)
    nextState = tf.nn.leaky_relu(tf.matmul(nextState1,wOutput)+bOutput)
    hRight.append(nextState)

hRight.reverse()
hRight = tf.concat(hRight,axis=1)    
#%%

sess = tf.InteractiveSession()
sess.run(tf.global_variables_initializer())
hRightTest =  sess.run(hRight,feed_dict={xInput:np.ones((3,200)),initState:np.zeros((3,1))})
